...
// SetMaxGoroutineNum 设置最多协程数量
// SetMaxGoroutineNum
func (s *Stream[T, Slice]) SetMaxGoroutineNum(num int) *Stream[T, Slice] {
	list := s.ToList()
	stream := ToStream(&list)
...
func (s *Stream[T, Slice]) DistinctBy(hash algorithm.HashComputeFunction[T]) *Stream[T, Slice] {
	...
	return ToStream(&res).SetPageSizeGetFunc(s.getPageSize).SetMaxGoroutineNum(s.maxGoroutineNum)
}
...
func (s *Stream[T, Slice]) OrderBy(desc bool, orderBy algorithm.HashComputeFunction[T]) *Stream[T, Slice] {
	...
	optionsStream := newOptionsStream[T, Options[T]](&data, s.getPageSize, s.maxGoroutineNum)
	...
	re := make([]any, 0)
	toStream := ToStream(&res).SetPageSizeGetFunc(s.getPageSize).SetMaxGoroutineNum(s.maxGoroutineNum)
...
// ParallelWithSetting 设置为并行模式
// ParallelWithSetting sets to parallel mode
